/**
 * dashboard-motorista.js — Painel de Requisições da Central de Motoristas.
 * Página dedicada (nova aba), replicando a lógica do dashboard da Retaguarda
 * (dashboard-ti.js), porém consumindo os dados de motorista via
 * action 'dashboard-ti-data' com destino: 'motorista'.
 *   - ES Module (escopo isolado, sem globals nem handlers inline).
 *   - Autenticação via X-Session-Token; endpoint requer sessão autenticada.
 */
...
// Chave de sessão própria para não colidir com a aba da Retaguarda.
const SESSION_KEY = 'bc-dashboard-motorista.session-token';
...
let _allTickets    = [];
// Item 1: filtro por motorista ('' = todos). Aplica-se a TODO o dashboard.
let _selectedMotorista = '';
...
function showLoginOverlay() {
  ...
  overlay.innerHTML = `
    <div class="dash-login-card">
      <div class="dash-login-brand">BUILD<span>.CONNECT</span> · MOTORISTAS</div>
...
// ── Filtro por motorista (Item 1) ───────────────────────────────────────────

/** Lista distinta de motoristas (responsáveis) presentes nos chamados. */
function extractMotoristas(tickets) {
  const map = new Map();
  (tickets || []).forEach(t => {
...
/** Filtra por motorista; '' retorna a lista inalterada. */
function filterByMotorista(tickets, motoristaId) {
  const id = String(motoristaId || '').trim();
  if (!id) return tickets;
  return (tickets || []).filter(t => String(t.atribuidoParaId || '').trim() === id);
}

/** Base do dashboard: aplica mês + motorista de uma vez. */
function getScopedTickets() {
  return filterByMotorista(filterByMonth(_allTickets, _selectedMonth), _selectedMotorista);
}
...
function normalizeTicket(t) {
  ...
  return {
    ...
    unidade:           t.unidade           ?? '—',
    // Motorista usa "Tipo de Serviço"; mantém alias categoria p/ reuso de helpers.
    tipoServico:       t.tipo_servico       ?? t.tipoServico       ?? '—',
    categoria:         t.tipo_servico       ?? t.tipoServico       ?? t.categoria ?? '—',
    ...
    // fotoUrls já chega resolvido (signed/Drive) do backend.
    fotoUrls:          Array.isArray(t.fotoUrls) ? t.fotoUrls : (Array.isArray(t.foto_urls) ? t.foto_urls : []),
...
// Unidade com maior número de solicitações no escopo atual.
function getTopUnidade(tickets) {
  const map = {};
  (tickets || []).forEach(t => { if (t.unidade) map[t.unidade] = (map[t.unidade] || 0) + 1; });
  const sorted = Object.entries(map).sort((a, b) => b[1] - a[1]);
...
function renderTable(rows) {
  ...
  return `<table>
    <thead><tr>
      ...
      <th>Unidade</th><th>Tipo de Serviço</th><th>Responsável</th><th>Aberto em</th><th>Duração</th><th>Fotos</th>
    </tr></thead>
    <tbody>
      ${rows.map(t => `<tr>
...
/** Célula de fotos: miniatura da 1ª foto (clicável) + contador, ou "—". */
function renderFotoCell(t) {
  const fotos = Array.isArray(t.fotoUrls) ? t.fotoUrls.filter(Boolean) : [];
  if (!fotos.length) return '—';
  const extra = fotos.length > 1 ? `<span class="dash-foto-count">+${fotos.length - 1}</span>` : '';
  return `<button type="button" class="dash-foto-thumb" data-dash-foto="${esc(t.id)}" title="Ver ${fotos.length} foto(s)">
    <img src="${esc(fotos[0])}" alt="Foto do chamado ${esc(t.id)}" loading="lazy" />${extra}
  </button>`;
}
...
function applyFilters() {
  ...
  const filtered =
    ...
    statusFilter === 'active' ? scoped.filter(t => t.status !== 'Concluído') :
    statusFilter === 'done'   ? scoped.filter(t => t.status === 'Concluído') :
  ...
  // Correção: ao reescrever a tabela, os listeners das miniaturas e os ícones
  // precisam ser reaplicados — sem isto, após filtrar as fotos não abrem o
  // lightbox e ícones não renderizam.
  bindFotoThumbs();
...
function onMotoristaChange(value) {
  _selectedMotorista = value || '';
  // Re-render completo: KPIs, gráficos e tabela passam a refletir só o motorista.
  renderDashboard({ _useCache: true });
}

/** Vincula o clique das miniaturas de foto (idempotente). */
function bindFotoThumbs() {
  document.querySelectorAll('[data-dash-foto]').forEach((btn) => {
    if (btn.dataset.bound === '1') return; // evita listeners duplicados
    btn.dataset.bound = '1';
    btn.addEventListener('click', () => {
      const id = btn.dataset.dashFoto;
      const ticket = _allTickets.find(t => t.id === id);
      const fotos = ticket && Array.isArray(ticket.fotoUrls) ? ticket.fotoUrls.filter(Boolean) : [];
      if (fotos.length) openLightbox(fotos, 0);
    });
  });
}
...
function attachDashboardListeners() {
  ...
  document.getElementById('motorista-select')
    ?.addEventListener('change', e => onMotoristaChange(e.target.value));
...
async function fetchData() {
  setRefreshing(true);
  try {
    const data = await requestApi('dashboard-ti-data', { period: 'tudo', destino: 'motorista' });
    if (data.success) {
...
function renderDashboard(data = {}) {
  ...
  const catByUnit       = countBy(all, t => `${t.unidade||'N/I'} · ${t.tipoServico||'N/I'}`);
  ...
  // Item 1: dropdown de motorista. Lista derivada de TODOS os chamados (não do escopo).
  const motoristaOptions   = extractMotoristas(_allTickets);
  const motoristaSelectHtml = `<option value="" ${_selectedMotorista === '' ? 'selected' : ''}>Todos os motoristas</option>`
    + motoristaOptions.map(m =>
        `<option value="${esc(m.id)}" ${m.id === _selectedMotorista ? 'selected' : ''}>${esc(m.nome)}</option>`
      ).join('');
  ...
  document.getElementById('root').innerHTML = `
    <div class="dash-header">
      <div class="dash-brand">
        <div>
          <div class="dash-brand-name">BUILD<span>.CONNECT</span> · MOTORISTAS</div>
          <div class="dash-subtitle">Última atualização: ${now}</div>
        </div>
      </div>
      <div class="dash-status">
        <div class="dash-month-filter">
          <label class="filter-label" for="motorista-select"><i data-lucide="user-check"></i> Motorista:</label>
          <select id="motorista-select" class="filter-select">
            ${motoristaSelectHtml}
          </select>
        </div>
        <div class="dash-month-filter">
    ...
    <div class="metrics-row">
      ...
      <div class="metric-card"><i data-lucide="building-2"></i>
        <div><span class="metric-value">${getTopUnidade(all)}</span><span class="metric-label">Unidade que mais solicita</span></div>
      </div>
    ...
    <div class="charts-row">
      <div class="chart-card">
        <div class="chart-head"><i data-lucide="tag"></i><span>Por Tipo de Serviço</span></div>
        ${colorBars(countBy(all, 'tipoServico'), COLORS_CAT)}
      </div>
      <div class="chart-card">
      ...
      <div class="chart-card">
        <div class="chart-head"><i data-lucide="layout-grid"></i><span>Tipo de Serviço por Unidade</span></div>
